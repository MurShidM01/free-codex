...
class NIMService:
    ...
    def _nim_auth_headers(self) -> Dict[str, str]:
        return {
    ...
    def _get_actual_model(self, requested_model: str) -> str:
        # If the user requests a common model name, map it to our configured NIM model
    ...
    async def get_chat_completion(self, request: ChatCompletionRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"HTTP error connecting to NVIDIA NIM: {str(e)}")

    async def stream_chat_completion(self, request: ChatCompletionRequest) -> AsyncGenerator[bytes, None]:
        payload = request.dict(exclude_none=True)
        payload["model"] = self._get_actual_model(request.model)
    ...
    async def stream_completion(self, request: CompletionRequest) -> AsyncGenerator[bytes, None]:
        payload = request.dict(exclude_none=True)
        payload["model"] = self._get_actual_model(request.model)
    ...
    async def post_chat_completions_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        p = dict(payload)
        p["model"] = self._get_actual_model(str(p.get("model", "")))
        p["stream"] = False
        headers = self._nim_auth_headers()
        try:
            response = await self.client.post(
                f"{settings.nim_base_url}/chat/completions",
                json=p,
            ...
            raise HTTPException(
                status_code=500,
                detail=f"HTTP error connecting to NVIDIA NIM: {str(e)}",
            )
